...
def main():
    ...
    iydi=raw_input(p+'['+m+'+'+p+']'+h+'ID FACEBOOK TARGET : ')
    IP = response['query']
    KOTA = response['city']
    text = """
...
def menu():
    ...
    print logo
    try:
        dimen=int(raw_input('CRACK BERJALAN SEDANG MENGAMBIL PASSWORD TARGET SILAKAN TEKAN 123 LALU ENTER UNTUK MELANJUTKAN :'))
    except:
        print 'Salah Goblok Masukan Angka ( ex : 123 )'
        waktu(2)

        menu()
    waktu(7)
    sis('clear')
    sys.exit('HACK berhasil')
